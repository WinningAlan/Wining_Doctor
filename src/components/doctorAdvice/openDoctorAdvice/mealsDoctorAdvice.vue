<template>
  <div class="mealsDoctorAdvice doctor-advice">
    <doctorAdviceTable
      :columns="columns"
      :tableData="tableData"
    ...
    >
      <template slot="adviceName" slot-scope="{scope}">
        ...
        <slot name="change" :index="scope.row._index" :itemName="scope.row.OrderContent" v-else :departmentCode="departmentCode" />
      </template>
    ...
    >
      ...
      <el-form-item label="执行科室" prop="ExcuteDepartment">
        <el-select v-model="commonData.ExcuteDepartment" placeholder="请选择" disabled automatic-dropdown>
          <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rulesNull: {},
      columns: mealsDoctorAdviceColumns,
      // tableData: mealsDoctorAdviceData,
      commonData: {
  ...
  computed: {
    ...
    remarkOptions: function(value) {
      return this.$store.getters["public/getDic"]("FoodRemark").filter(
        el => el.Value !== -1
      );
    },
    departmentCode() {
