<template>
  <div class="nurseDoctorAdvice doctor-advice">
    
    <doctorAdviceTable :columns="columns" :tableData="tableData" row-key="DictItemCode" :orderType="5">
      <template slot="adviceName" slot-scope="{scope}">
        <template v-if="!scope.row._edit">{{scope.row.OrderContent}}</template>
        <slot name="change" :index="scope.row._index" :itemName="scope.row.OrderContent" :departmentCode="departmentCode" v-else />
      </template>
    ...
    >
      ...
      <el-form-item label="执行科室" prop="ExcuteDepartment">
        <el-select v-model="commonData.ExcuteDepartment" placeholder="请选择" disabled>
          <el-option
...
<script>
import { nurseDoctorAdviceColumns } from "../../../module/tableColumns";
import { nurseDoctorAdviceData } from "../../../module/tableData";
import doctorAdviceTable from "./doctorAdviceTable.vue";
import { GetFormatDateTime } from "@/utils/public";
import { mapGetters } from "vuex";

export default {
  name: "nurseDoctorAdvice", //护理医嘱控件
  components: {
    doctorAdviceTable
  },
    props: {
  ...
  data() {
    return {
      ...
      rulesNull:{},
      columns: nurseDoctorAdviceColumns,
      // tableData: nurseDoctorAdviceData,
      commonData: {
  ...
  created() {
    ...
    this.nowTime = this.commonData.StartTime =this.commonData.StartTime?this.commonData.StartTime: GetFormatDateTime();
        ...
        // this.nowTime = this.commonData.StartTime = GetFormatDateTime();
  ...
  computed: {
    ...mapGetters({
      ...
      return this.$store.getters['public/getDic']("NursingRemark").filter(el=>el.Value!==-1);
    },
    departmentCode() {
